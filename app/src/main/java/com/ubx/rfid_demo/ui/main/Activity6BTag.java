...
//                    Log.d(TAG, "initRfidService: outputPower = " + outputPower);
                    ...
                    Log.d(TAG, "initRfidService: El estado falló.");
                }
            }
        });
    ...
    public void onClick(View view) {
        getRFIDManager();
        if (mRfidManager == null) {
            toast("rfid 未初始化.");//no inicializado
            return;
        }
        ...
        if (view == rButton) {
            ...
            if (result != 0) {
                toast("指令执行失败");//La ejecución del comando falló
            }
    ...
    private ITag6BCallback iTag6BCallback = new ITag6BCallback() {
        ...
        public void onRead6BTag(String strData) {
            toast("读取成功：" + strData);//Leer con éxito:
        ...
        public void onLock6BTag(byte nStatus) {
            int result = nStatus;
            if (result != 0) {
                toast("锁定失败");//Error de bloqueo
            } else {
                toast("锁定成功");//Bloqueo exitoso
            }
        }
        ...
        public void onExeCMDStatus(byte cmd, byte status) {
            ...
            if (cmd == CMDCode.ISO18000_6B_WRITE_TAG) {
                if (status == ErrorCode.SUCCESS) {
                    toast("写成功");//Escribe con éxito
                } else {
                    toast("写失败");//Error de escritura
                }
            }
        }
